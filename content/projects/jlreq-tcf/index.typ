...
- #strong[`\footnoteL{<text>}`]
- Typesets a footnote in the #strong[left-hand] column.
- Automatically increments the internal footnote counter.

- #strong[`\footnoteR{<text>}`]
- Typesets a footnote in the #strong[right-hand] column.
- Automatically increments the internal footnote counter.
...
- #strong[`\footnotemarkL[<num>]`] / #strong[`\footnotemarkR[<num>]`]
- Prints the footnote mark for the left (L) or right (R) column.
- #strong[With `[<num>]`]: Uses the integer `<num>` as the mark number. Does #strong[not] increment the internal counter.
- #strong[Without `[<num>]`]: Increments the internal counter and uses that value.

- #strong[`\footnotetextL[<num>]{<text>}`] / #strong[`\footnotetextR[<num>]{<text>}`]
- Defines the footnote text for the left (L) or right (R) column without printing a mark in the main text.
- #strong[With `[<num>]`]: Uses `<num>` as the label in the footnote area.
- #strong[Without `[<num>]`]: Uses the current value of the internal counter.
...
+ #strong[Layout Emulation]:
This package attempts to reproduce the footnote layout (rules, spacing, indentation) of the `jlreq` class. However, this is an #strong[emulation] created using `minipage` environments and manual spacing. It does not strictly inherit the internal typesetting logic of `jlreq`. There may be slight visual discrepancies depending on the document settings.
+ #strong[Conflict with Standard `\footnote`]:
#strong[Do not use the standard `\footnote` command on the same page as `\footnoteL` or `\footnoteR`.]
Since this package manages its own output routine for two-column footnotes, using the standard `\footnote` command simultaneously will result in #strong[two separate footnote blocks] appearing at the bottom of the page (one handled by LaTeX's standard mechanism and one by this package). This usage is not supported.
+ #strong[Long Footnotes]:
