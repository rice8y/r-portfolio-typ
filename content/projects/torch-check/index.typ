...
#show: project.with(
  ...
  languages: ("Rust",),
  links: (
    (label: "GitHub", url: "https://github.com/rice8y/torch-check"),
    (label: "crates.io", url: "https://crates.io/crates/torch-check"),
  ),
)
...
It deliberately does #strong[not] treat the `CUDA Version` field printed by `nvidia-smi` as a strict upper bound. Compatibility is based on the installed driver, NVIDIA's CUDA major-family minor-version compatibility rules, the exact wheel tags, glibc, and conservatively maintained GPU-architecture evidence.
...
From #link("https://crates.io/crates/torch-check")[crates.io]:
...
Prebuilt archives are published through #link("https://github.com/rice8y/torch-check/releases")[GitHub Releases]:

#table(
  columns: (1.25fr, 1.25fr, 2.5fr),
  ...
  [Ubuntu 22.04.x x86_64, including 22.04.2],
  [`x86_64-unknown-linux-musl`],
  [Native Ubuntu 22.04 smoke test; static musl binary; full production recommendation support],

  [Ubuntu 22.04 ARM64],
  [`aarch64-unknown-linux-musl`],
  [Native smoke test; inspection only],

  [macOS Intel],
  [`x86_64-apple-darwin`],
  [Native test on macOS 15; inspection only; deployment target 10.15],

  [macOS Apple Silicon],
  [`aarch64-apple-darwin`],
  [Native test on macOS 15; inspection only; deployment target 11.0],

  [Windows x64],
  [`x86_64-pc-windows-msvc`],
  [Native test on Windows Server 2022; inspection only; static CRT],
)
...
- #link("https://docs.nvidia.com/deploy/cuda-compatibility/minor-version-compatibility.html")[NVIDIA CUDA minor-version compatibility]
- #link("https://docs.nvidia.com/cuda/archive/13.2.0/cuda-toolkit-release-notes/index.html#cuda-driver")[NVIDIA CUDA release driver table]
- #link("https://download.pytorch.org/whl/")[PyTorch official wheel index]
- #link("https://github.com/pytorch/pytorch/blob/main/RELEASE.md")[PyTorch release matrix]
- #link("https://pytorch.org/get-started/previous-versions/")[Previous PyTorch versions]
- #link("https://github.com/pytorch/pytorch/blob/v2.6.0/.ci/manywheel/build_cuda.sh")[PyTorch 2.6.0 Linux wheel build configuration]
- #link("https://github.com/pytorch/pytorch/blob/v2.7.0/.ci/manywheel/build_cuda.sh")[PyTorch 2.7.0 Linux wheel build configuration]
- #link("https://github.com/pytorch/pytorch/blob/v2.7.1/.ci/manywheel/build_cuda.sh")[PyTorch 2.7.1 Linux wheel build configuration]
- #link("https://github.com/pytorch/pytorch/blob/v2.8.0/.ci/manywheel/build_cuda.sh")[PyTorch 2.8.0 Linux wheel build configuration]
- #link("https://github.com/pytorch/pytorch/blob/v2.9.0/.ci/manywheel/build_cuda.sh")[PyTorch 2.9.0 Linux wheel build configuration]
- #link("https://github.com/pytorch/pytorch/blob/v2.9.1/.ci/manywheel/build_cuda.sh")[PyTorch 2.9.1 Linux wheel build configuration]
- #link("https://github.com/pytorch/pytorch/blob/v2.10.0/.ci/manywheel/build_cuda.sh")[PyTorch 2.10.0 Linux wheel build configuration]
- #link("https://github.com/pytorch/pytorch/blob/v2.11.0/.ci/manywheel/build_cuda.sh")[PyTorch 2.11.0 Linux wheel build configuration]

== JSON contract

Every JSON response contains `"schema_version": 1`. Reason and warning fields use stable codes and parameters rather than terminal prose. Successful command schemas and the common error envelope are in #link("https://github.com/rice8y/torch-check/blob/main/data/schemas")[`data/schemas/`]. Complete JSON reports, including structured diagnostics, are written to stdout. Human-mode errors and failures to serialize a JSON document are written to stderr; ordinary JSON-mode failures still produce the error envelope on stdout.
...
`torch-check` has no telemetry and does not upload detected environment data. Network access is limited to downloading public package metadata. It never runs an installation command. External probes use argument arrays rather than a shell, with time and output limits. See #link("https://github.com/rice8y/torch-check/blob/main/SECURITY.md")[SECURITY.md] for reporting issues.
...
Copyright © 2026 Eito Yoneyama. Licensed under either the #link("https://github.com/rice8y/torch-check/blob/main/LICENSE-MIT")[MIT License] or the #link("https://github.com/rice8y/torch-check/blob/main/LICENSE-APACHE")[Apache License, Version 2.0], at your option.